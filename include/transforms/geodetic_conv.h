...
public:
	...
	void getENUorigin(double* east, double* north, double* up)
	{
		*east = initial_east_;
		*north = initial_north_;
		*up = initial_up_;
	}

	void initialiseReference(const Eigen::Vector3d ref_origin)
	{
		// Save NED origin
		double latitude = ref_origin.x();
		double longitude = ref_origin.y();
		double altitude = ref_origin.z();
		initial_latitude_ = deg2Rad(latitude);
		initial_longitude_ = deg2Rad(longitude);
		initial_altitude_ = altitude;
		geodetic2Enu(latitude, longitude, altitude, &initial_east_, &initial_north_, &initial_up_);
...
private:
	...
	/// Function that returns the conversion matrices between NED and ECEF
	/// \param lat_radians
	/// \param lon_radians
	/// \return Eigen::Matrix3d of conversion matrix
	...
	/// Intial GPS values used when geodetic object is set
	double initial_latitude_;
	double initial_longitude_;
	double initial_altitude_;

	/// Intial ENU values used when geodetic object is set
	double initial_north_;
	double initial_east_;
	double initial_up_;

	/// Intial ECEF values used when geodetic object is set
